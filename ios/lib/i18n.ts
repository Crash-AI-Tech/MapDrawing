...
export const translations = {
  ...
  unblock: { zh: '解除', en: 'Unblock', ja: '解除' },
  unblockConfirm: { zh: (name: string) => `解除对 ${name} 的屏蔽？你将重新看到对方的图钉和绘画。`, en: (name: string) => `Unblock ${name}? You will see their pins and drawings again.`, ja: (name: string) => `${name} のブロックを解除しますか？ピンと描画が再び表示されます。` },
  unblockFailed: { zh: '解除屏蔽失败，请重试。', en: 'Failed to unblock user. Please try again.', ja: 'ブロック解除に失敗しました。もう一度お試しください。' },
  blockedOn: { zh: (date: string) => `屏蔽于 ${date}`, en: (date: string) => `Blocked ${date}`, ja: (date: string) => `${date} にブロック` },
  ...
  passwordMin8: { zh: '密码（至少8位）', en: 'Password (min 8 characters)', ja: 'パスワード（8文字以上）' },
  ...
  passwordTooShort: { zh: '密码至少需要8个字符', en: 'Password must be at least 8 characters', ja: 'パスワードは8文字以上必要です' },
  ...
  newPasswordMin8: { zh: '新密码（至少8位）', en: 'New Password (min 8 characters)', ja: '新しいパスワード（8文字以上）' },
