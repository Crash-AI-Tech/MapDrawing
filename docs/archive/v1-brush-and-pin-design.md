...
> **历史文档：** v2 运行时仅支持铅笔与橡皮擦，不再兼容 marker/spray/highlighter 数据。当前协议以 `packages/shared/src/protocol.ts` 为准。
...
应你的决策，笔刷支持策略已调整为：**仅保留 `pencil`（铅笔）和 `eraser`（橡皮擦）**，不再支持 `spray`、`highlighter`、`marker` 三种笔刷。

理由与要点：

- 简化用户选择，降低认知成本，突出「快速标记与擦除」的核心场景；
- 降低跨平台实现与维护成本（只需保证两端的铅笔与橡皮擦一致）；
- 移除高成本渲染和同步（喷枪的粒子、大面积半透明叠加的高性能代价）；
- 将笔画大小与透明度控件从独立笔刷面板移动到颜色选择面板，统一控制体验。

### 2.2 功能行为调整（交互说明）

- 主笔画按钮（工具栏的笔画模式）在点击后作用为 **在 `pencil` 与 `eraser` 之间切换**（即：单一按钮负责切换两者）；
- 颜色选择面板中新增/包含 `大小（Size）` 与 `透明度（Opacity）` 控件，作为铅笔与橡皮擦的共同属性入口；
- UI 层面：`BrushPanel` / 笔刷选择应只显示铅笔与橡皮擦两项（或仅保留一个切换按钮），并移除 `spray`、`marker`、`highlighter` 的入口与图标。
...
`@mapdrawing/contracts` 中的 `BRUSH_IDS` 已包含 5 种笔刷 ID：
